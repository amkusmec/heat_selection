...
# models <- read_csv("data/regression_data/selected_models.csv")
basis <- c("CUB", "STEP")


# for (j in 1:nrow(models)) {
#   reg_dat <- paste0("data/regression_data/reg_dat_", 
#                     models$Basis[j], ".rds") %>% 
#     read_rds()
...
for (b in basis) {
  ...
  # Set up formulae
  # fix_form <- paste0("LogYield ~ 1 + bs(PRCP, df = ", 
  #                    models$PRCP_df[j], ") + ", 
  #                    paste(bn, collapse = " + "))
  # if (models$Covariate[j] != "None") fix_form <- paste0(fix_form, " + ", models$Covariate[j])
  # 
  # rand_form <- paste0("(1 | County) + (0 + poly(Year, degree = ", 
  #                     models$Year_deg[j], ", simple = TRUE) | State) + ",  
  ...
  res <- mclapply(1:ncol(boots), function(i) {
      # cat(models$Basis[j], "-- Bootstrap", i, "\n", 
      cat(b, "-- Bootstrap", i, "\n", 
      ...
      # saveRDS(model, paste0("data/bootstraps/", models$Basis[j], 
